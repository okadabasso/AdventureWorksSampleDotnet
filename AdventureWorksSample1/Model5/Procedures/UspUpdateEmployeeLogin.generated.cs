...
namespace Sample.Entities
{
    public partial class UspUpdateEmployeeLogin
    {
        ...
        )
        {
            ...
            var affected = context.Database.ExecuteSqlCommand(@"EXEC @return_value = [HumanResources].[uspUpdateEmployeeLogin] @BusinessEntityID = @BusinessEntityID, @OrganizationNode = @OrganizationNode, @LoginID = @LoginID, @JobTitle = @JobTitle, @HireDate = @HireDate, @CurrentFlag = @CurrentFlag",
                returnValue, p0, p1, p2, p3, p4, p5);
            ...
            bool? currentFlag        )
        {
            ...
            var result = context.Database.SqlQuery<T>(@"EXEC [HumanResources].[uspUpdateEmployeeLogin] @BusinessEntityID = @BusinessEntityID, @OrganizationNode = @OrganizationNode, @LoginID = @LoginID, @JobTitle = @JobTitle, @HireDate = @HireDate, @CurrentFlag = @CurrentFlag"
                , p0, p1, p2, p3, p4, p5);
